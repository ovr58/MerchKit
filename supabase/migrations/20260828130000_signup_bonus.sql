...
-- Сначала профили. Триггер `on_auth_user_created` (веха M2) заводит профиль только новым
-- пользователям, а в окружении, где аккаунты появились раньше самой таблицы, их профилей
-- нет вовсе. На локальном стенде это незаметно — база всегда пересоздаётся с нуля, — но на
-- стейдже накат упал именно здесь: аккаунты M2 регистрировались, когда `profiles` там ещё
-- не было. Догонка без этого шага пытается начислить баллы владельцу, которого нет.
