 ...
 * прогоны 2026-08-30). Прежние 60 с оставляли бы над худшим наблюдённым вызовом восемь
 * секунд запаса — то есть резали бы успешные генерации на первом же медленном ответе.
 *
 * **Накладные нашего стека сюда больше не заложены.** Первая редакция этого потолка стояла
 * на 150 с, потому что B13 приписывал конвейеру ~32 с сверх провода. На подтверждающем
 * прогоне 2026-08-31 разрыв не воспроизвёлся: против вендорского лога наши записи разошлись
 * на 1,2–5,8 с (20 981 против 26 775; 39 448 против 41 921; 23 789 против 24 941; 12 923
 * против 14 999). Потолок сведён к 120 с — вдвое над худшим наблюдённым вызовом, без запаса
 * на разрыв, существование которого под вопросом.
 *
 * Жёсткого обещания по времени генерации в `docs/TZ.md` нет (NFR-02 требует видимого
 * прогресса, а не срока), поэтому подъём с 60 с контракта не ломает. Считается на **каждый**
 * вызов: отказ по содержанию и повтор на резервной модели — два вызова подряд, и суммарное
 * ожидание пользователя на этом пути складывается из обоих (замерено 23,8 + 12,9 с).
 */
