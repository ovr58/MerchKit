...
function imagePrompt(
  ...
  // знаком. Язык задан явно: на англоязычном товаре блок уходил в английский (замер шага 2
  // плана planning/active/card-text-block_2026-08-30.md — 3 кадра из 5 на наборе food-pepsi).
  const layoutLine = kind === 'card'
    ? `На изображении, помимо товара, нужен аккуратный текстовый блок: название ` +
      `«${product.title}» и краткое описание — рядом с товаром, не поверх него. Никакого ` +
      'другого текста в кадре: ни дополнительных полей и характеристик, ни подписей в ' +
      'квадратных скобках, ни шаблонных заготовок, которые надо заполнять. Никаких логотипов, ' +
      'водяных знаков и названий магазинов, которых нет на фото товара. Весь текст блока — ' +
      'на русском языке, даже если сам товар и надписи на упаковке англоязычные.'
