...
> свойств, `Save` публикует новую версию для всех). Пересборка из исходников — заходом
> `--extract` по процедуре скила `design`; отдельной копии макетов в репозитории нет
> намеренно, источник правды — канвас.
