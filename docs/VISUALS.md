...
- **Иконки** и **шрифты** — пустые базы с показанной формой записи; объяснено, почему список
  не назначается заранее, а вырастает из заявок при разборе.
- **Круг пополнения** — схема из шести шагов: образец → разбор → заявки → дедуп → заполнение
  → доступно макетам, и снова.

**Страница отражает состояние на 2026-08-31 и переиздаётся по мере наполнения баз** —
счётчики в шапке и статусы ячеек правятся в исходнике, URL остаётся прежним.
