...
| [V-11](#v-11) | Конвейер карточки | Интерактивная страница | Разбор образца на слои, шесть этапов сборки и принятые решения — объяснение для нетехнического читателя | `card-assembly-pipeline_2026-08-31.md` |
| [V-12](#v-12) | Картотека конвейера | Интерактивная страница | Инвентарь баз образцов, иконок и шрифтов: 21 ячейка «категория × площадка», круг пополнения, что пусто | `card-assembly-pipeline_2026-08-31.md` шаг A7 |
| [V-13](#v-13) | Пересборка трёх образцов | Интерактивная страница | Гейт фазы A: оригинал, пересборка поверх него и голый стек слоёв — рядом, на трёх образцах | `card-assembly-pipeline_2026-08-31.md` шаг A3 |
...
stateDiagram-v2
    ...
    state "Мастер генерации" as W {
        ...
        Тип --> Показ
        Показ --> Показ: знак не подошёл (B3)
        Показ --> Запуск
...
>
> **Логотип загружается на шаге «Как показать» и только у карточки** (шаг B3): у генерации
> типа «фото» макета нет, ставить знак некуда. Негодный файл возвращает на тот же шаг, как
> и непринятое фото; отсутствие знака шаг не блокирует — на шаге «Запуск» об этом сказано, а
> слой с логотипом снимается на сборке правилом K-3.
>
...
Что из этого войдёт в промпт, а что останется на усмотрение провайдера — решалось на M4 и
уточнялось на M5. **Веха M7 сняла этот вопрос:** вёрстку ставит наш сборщик, а не промпт
([ADR-0012](adr/0012-card-layout-is-ours-not-vendors.md)), поэтому подкладывать образцы
провайдеру больше незачем.

**Рядом с оригиналами лежат сборки гейта A3** (2026-08-31, шаг A3 плана
[`card-assembly-pipeline_2026-08-31.md`](../planning/active/card-assembly-pipeline_2026-08-31.md)):
`*.rebuilt.png` — наши слои поверх оригинала, проверка попадания разбора; `*.layers.png` —
те же слои без кадра, проверка полноты. Оба перезаписываются командой
`npm run cards:roundtrip`, разборы лежат в
[`tools/card-pipeline/samples/`](../tools/card-pipeline/samples/).

## V-11 — Конвейер карточки

**Страница:** https://claude.ai/code/artifact/a6e753e8-8f25-4d98-9786-01a01ce98c25
**Исходник:** [`design/card-pipeline/`](design/card-pipeline/) — с плейсхолдером вместо
встроенной картинки, сборка одной командой (см. README папки).

Объяснение конвейера сборки для **нетехнического читателя** — сделано по просьбе пользователя
2026-08-31 как материал для пересказа заказчику. Не дублирует план
[`card-assembly-pipeline_2026-08-31.md`](../planning/active/card-assembly-pipeline_2026-08-31.md):
план — что делать, страница — почему это устроено именно так.

Что показывает:

- **разбор образца V-10 на пять слоёв** — с самой карточкой рядом со схемой стопки; отдельно
  разобран приём «полупрозрачная плашка поверх рукава», ради которого конвейер и затевается;
- **сравнение «одна нейросеть против конвейера»** — почему надписи ставит программа, а не
  модель, с замеренной частотой брака;
- **две дорожки** — разовая подготовка библиотеки макетов и сборка на каждой генерации;
- **шесть этапов** с блоками «Вопрос к вам» и **13 вопросов одним списком** в конце.

**Переиздана 2026-08-31 по тому же URL:** блоки «Вопрос к вам» заменены на «Решено» с
ответами пользователя, добавлены решение о порядке вех (M7 перед M6) и честный абзац про
анатомию рук. В конце — 13 принятых решений и 3 оставшихся вопроса. Первичная редакция
задавала вопросы; текущая отвечает на них и годится для показа заказчику.

## V-12 — Картотека конвейера

**Страница:** https://claude.ai/code/artifact/4ad42285-d62a-4247-9d6a-72d1a94fcdf7
**Исходник:** [`design/card-library/`](design/card-library/) — самодостаточный HTML, сборки
не требует.

Инвентарь библиотеки макетов и двух баз ассетов офлайн-конвейера (шаги A7 и B0 плана
[`card-assembly-pipeline_2026-08-31.md`](../planning/active/card-assembly-pipeline_2026-08-31.md)).
Не отчёт о достижениях, а рабочий инструмент пополнения: пустые слоты видны так же ясно, как
заполненные.

- **Макеты** — 34 проверенных `CardLayout` синхронизированы в `card_layouts`; один помечен
  универсальным, чтобы подбор не падал на пустом совпадении. Рабочая копия разборов остаётся
  в `tools/card-pipeline/samples/`; теги категории, площадки, сценария и скрытых кистей
  подтверждены B2.0 и записаны в базу.
- **Рабочая копия образцов** — 7 категорий × 3 площадки = 21 ячейка «папками с файлами»; на
  каждой ячейке метки требований площадки (3 : 4 · серый фон Ozon · квадрат Ozon Fresh).
  Отдельно полка гейта: три файла V-10.
- **Иконки** и **шрифты** — заполнены на шаге A5: две иконки из образца с курткой (рисунок
  показан прямо в строке) и пять начертаний двух гарнитур на пять ролей, с лицензией в
  строке. Объяснено, почему список не назначается заранее, а вырастает из заявок при разборе.
- **Круг пополнения** — схема из шести шагов: образец → разбор → заявки → дедуп → заполнение
  → доступно макетам, и снова.

**Переиздана 2026-09-01 после B2.0:** счётчики основаны на локальной библиотеке (34 строки,
один fallback); предложения Batch API прошли визуальное ревью и подтверждены пользователем.
Дальше страница переиздаётся по мере наполнения библиотек — URL остаётся прежним.

## V-13 — Пересборка трёх образцов

**Страница:** https://claude.ai/code/artifact/0c826cf1-2e51-4fb8-91af-2095b2b350c6
**Исходник:** [`design/card-roundtrip/`](design/card-roundtrip/) — с плейсхолдерами вместо
девяти встроенных картинок, сборка одной командой (см. README папки).

Гейт фазы A вехи M7 (шаг A3 плана
[`card-assembly-pipeline_2026-08-31.md`](../planning/active/card-assembly-pipeline_2026-08-31.md)).
Страница существует ради одного решения пользователя: композиция узнаётся — начинается отбор
21 образца; не узнаётся — язык макета переделывается.

Что показывает:

- **три образца V-10 в три колонки каждый** — оригинал · наши слои поверх оригинала
  (оригинал служит кадром вендора, поэтому промах в разборе виден как двоение) · те же слои
  без кадра;
- **что каждый образец проверил** в языке — модули-группы, полупрозрачные плашки, градиентную
  колонку, образцы цвета, вылет за обрез, переменный z-порядок;
- **честную границу гейта** — отдельным блоком то, чего он не доказывает: совпадение
  гарнитур, качество дизайна, автоматический разбор, поведение на живом вендоре;
- **таблицу слоёв, снятых правилом K-3** на этом самом прогоне.

Переиздаётся по тому же URL после `npm run cards:roundtrip`, если разборы менялись.
